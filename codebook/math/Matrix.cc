...
template <class T> struct matrix {
  ...
  template <size_t rows, size_t cols> matrix(T (&init)[rows][cols]) {
    r = rows;
    c = cols;
    mat.resize(r, vector<T>(c));
    for (int i = 0; i < r; i++)
...
template <class T> matrix<T> operator^(const matrix<T> &a, unsigned int n) {
  if (n == 0)
    return eye<T>(a.r);
  if (n % 2 == 0)
    return (a * a) ^ (n / 2);
...
// returns a^1 + a^2 + ... + a^n
template <class T> matrix<T> powsum(const matrix<T> &a, unsigned int n) {
  if (n == 0)
    return matrix<T>(a.r, a.r);
  if (n % 2 == 0)
